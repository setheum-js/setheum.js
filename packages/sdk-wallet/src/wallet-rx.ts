...
import { eventsFilterRx, FixedPointNumber, Token, TokenBalance } from '@setheum-js/sdk-core';
...
export class WalletRx extends WalletBase<ApiRx> {
  ...
  private _queryPrice = memoize(
    (currency: MaybeCurrency, at?: number): Observable<PriceData> => {
      ...
      if (currencyName === 'USDJ' || currencyName === 'KUSD') {
        const usd = this.tokenMap.get('USDJ') || this.tokenMap.get('KUSD') || new Token('USD', { decimal: 12 });
      ...
      switchMap(() => {
        ...
        return this.api.rpc.chain.getBlockHash(at).pipe(
          switchMap((hash) => {
            ...
            return this.api.query.setheumOracle.values.at<Option<TimestampedValue>>(hash, currencyId).pipe(
              map((data) => {
  ...
  public queryPriceFromDex = memoize(
    (currency: MaybeCurrency, at?: number): Observable<PriceData> => {
      ...
      const usd = this.tokenMap.get('USDJ') || this.tokenMap.get('KUSD');
