import { AnyApi, createDexShareName, FixedPointNumber, forceToCurrencyName, MaybeCurrency, Token } from '@setheum.js/sdk-core';
...
export class Liquidity implements BaseSDK {
  ...
  private subscribeBaseTokenPrice = memoize((token: MaybeCurrency): Observable<FixedPointNumber> => {
    ...
    if (name === 'SETM') {
      return this.subscribePoolPositions(createDexShareName('SETUSD', 'SETM')).pipe(
        map((positions) => {
    ...
    if (name === 'SERP') {
      return this.subscribePoolPositions(createDexShareName('SETUSD', 'SERP')).pipe(
        map((positions) => {
          return positions.amounts[0].div(positions.amounts[1]);
        })
      );
    ...
    if (name === 'DNAR') {
      return this.subscribePoolPositions(createDexShareName('SETUSD', 'DNAR')).pipe(
        map((positions) => {
          return positions.amounts[0].div(positions.amounts[1]);
        })
      );
    ...
    if (name === 'HELP') {
      return this.subscribePoolPositions(createDexShareName('SETUSD', 'HELP')).pipe(
        map((positions) => {
          return positions.amounts[0].div(positions.amounts[1]);
        })
      );
  ...
  public subscribeDexPrice = memoize((token: Token) => {
    const baseTokens = ['SETM', 'SERP', 'DNAR', 'HELP', 'SETR', 'SETUSD'];
    const name = forceToCurrencyName(token);
    ...
    if (name === 'SETR') {
      return of(FixedPointNumber.fromRational(4, 1));
    }
